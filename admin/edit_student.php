...
try {
    ...
    $required = ['student_id', 'first_name', 'last_name', 'email', 'college_id', 'course_id', 'year_level', 'original_student_id'];
    ...
    if ($email !== $currentData['email']) {
        $stmt = $pdo->prepare("SELECT 1 FROM students WHERE email = ? AND student_id != ?");
        $stmt->execute([$email, $originalId]);
        if ($stmt->fetch()) {
    ...
    if (!empty($cor_number) && $cor_number !== $currentData['cor_number']) {
        $stmt = $pdo->prepare("SELECT 1 FROM students WHERE cor_number = ? AND student_id != ?");
        $stmt->execute([$cor_number, $originalId]);
        if ($stmt->fetch()) {
